...
class Programme {
  ...
  /// Optimized Programme parsing - replaces O(n) findElements with O(1) map lookup
  factory Programme.fromXmlElement(XmlElement element) {
    ...
    // OPTIMIZATION: Build a map of child elements for O(1) lookup instead of O(n) traversal
    // This replaces 30+ findElements/getElement calls with a single iteration
    final childrenMap = <String, List<XmlElement>>{};
    for (final child in element.childElements) {
      final name = child.name.local;
      (childrenMap[name] ??= []).add(child);
    }

    List<XmlElement> getMultiple(String name) => childrenMap[name] ?? const [];
    ...
    return Programme(
      ...
      titles: getMultiple('title')
          .map(Title.fromXmlElement)
          .toList(growable: false),
      descs:
          getMultiple('desc').map(Desc.fromXmlElement).toList(growable: false),
    );
  }
